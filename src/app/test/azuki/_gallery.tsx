...
import type { ERC721Metadata } from "../../api/azuki/types";
    ...
    <>
      {items?.slice(0, visibleScope).map((item, index) => {
        ...
        return (
          ...
          >
            <NFTImage name={name} imageUrl={imageUrl} />
            <div className="flex flex-col gap-1 px-2 py-3">
              <p className="tracking-wider uppercase font-mono">{name}</p>
              <p className="text-sm uppercase font-mono opacity-50">
                0xabcd...888
              </p>
            </div>
          </div>
...
const NFTImage = ({ name, imageUrl, ...props }: NFTImageProps) => {
  ...
  return (
    <div
      ...
      style={{
        boxShadow:
          "0 1px 1px rgba(0,0,0,.075),0 2px 2px rgba(0,0,0,.075),0 4px 4px rgba(0,0,0,.075),0 8px 8px rgba(0,0,0,.075),0 16px 16px rgba(0,0,0,.075)",
      }}
      {...props}
    >
      <Skeleton
